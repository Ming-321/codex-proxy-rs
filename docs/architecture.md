...
冻结到其 car 分组。seat 是共享预算、并发和 RPM 的 owner，Key 保留鉴权、客户端身份与用量明细。请求开始时
...
拼车整页草稿由 Admin 校验并准备凭据，Store 在一个配置事务内保存分组、车位与 Key。
全局配置版本与周期观测时间共同防止旧草稿覆盖新配置；请求指纹与事务回执保证响应丢失后重试不重复创建 Key。
每辆车独立选择手动、跟随周期或自动额度策略；自动分配按固定分母或精确的车位数量比例计算，空车位及停用车位保留份额。
...
普通 Key 的日金额、七天金额、并发和 RPM 跨账号、跨 Provider 合计，零表示不限；seat 成员统一使用车位限制，不另叠加 Key 限制。修改限额不重置已用金额。
...
账号容量预测属于 Admin 的只读派生规则，不修改 quota 权威状态或金额结算。car周期Worker可消费有效美元预测，
按独立的发布策略调整本地容量，但不能把预测当成上游真实余额。Store通过专用采样端口在同一SQL快照内返回
截至观测时间的累计数值及有界历史Provider文档。原生 Provider 复用协议解析器解释文档，插件由 Runtime 本地解释
SDK 声明的版本化额度观测，不在历史查询中调用插件进程。观测通过 Core 原有请求结算持久化，
账号关联由实际执行上下文确定，窗口身份与归属必须匹配当前额度窗口。
每个采样点至多附带两个抽样前相邻文档；未解析或同完成时间的邻居不能证明连续性。
Admin仅在前后原始观测匹配同一窗口、套餐与非下降比例时忽略孤立重置时间异常，不修改Provider权威快照；
再按中立的额度事实选择近期进度段预测剩余量，加本周期已记录用量形成周期总量。
